...
# CORRECTION_1: exportselection=False was added to clear IndexError: tuple index out of range.

# NOTE: This code only displays prepopulated "albums" and "songs"
# We will go to jukebox6 to link the fields together so that when you click artist,
# it will load their albums and when you click album, it loads their songs

# ==================================
# DataListBox Class code
# ==================================

# In jukebox4.py, we have produced a scrollable subclass of TKinter listbox class and then populated the list from music database
# We will now move the database functionality into another subclass to produce a listbox that automatically displays data from a database table

# We will need to decide how to initialize the instances of our new class.
# We will need to tell it the name of the table that it should get the data from and which field it needs to display.
# So its a good idea to provide a database connection so our classes don't rely on a global variable.

# A sort order is also useful. A song should be listed in track order i.e. order they appear on the album, but we will display the song titles.
# We can make the default sort order the displayed field, so if a different sort order is not specified the list will sort items alphabetically.

# At the moment, when we run the program, only first column (Artists) displays data to start with.
# So we need a way to trigger the initial load of data for those instances that will display data as soon as they are created
# We also need a way to get the other listboxes to load their data once we know the ID of the artist or album.

# It would make sense to use the same method in both cases and just pass NONE for the ID when we are loading the first list.
# We will create the DataListBox Class: CHANGE_21
